...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            maxpos = d.argMax()
            tempDistance = self.distancer.getDistance(pacmanPosition,d.argMax())

            if minDistance== None or tempDistance < minDistance:
                minDistance = tempDistance
                ghostPosition = maxpos
        
        minDistance = None
        for act in legal:
            successors = Actions.getSuccessor(pacmanPosition,act) 
            temp = self.distancer.getDistance(successors,ghostPosition)
            
            if minDistance == None or temp < minDistance: 
                minDistance = temp 
                action = act
