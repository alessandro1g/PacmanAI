...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        #raiseNotDefined()
        """
        which draws a sample from the distribution, where 
        ...
        curr = 0 
        for k,v in self.items(): 
            if curr <= rand and rand < curr+(v/total):
                return k 
            curr += v/total
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        #raiseNotDefined()
        dist = self.beliefs
        pPosition = gameState.getPacmanPosition()
        ...
        for p in self.allPositions: 
            probability = self.getObservationProb(observation,pPosition, p, jPosition) 
            dist[p] =  probability * self.beliefs[p]
            #print(dist[p])
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        dist = DiscreteDistribution() 
        for prior in self.allPositions:
            tempPosDistribution = self.getPositionDistribution(gameState,prior)
            for new in self.allPositions:
                if tempPosDistribution[new]>0:
                    dist[new] += tempPosDistribution[new] *self.beliefs[prior]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***" 
        ppp = self.numParticles//len(self.legalPositions)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for g in self.particles:
            dist[g] += self.getObservationProb(observation,pPosition,g,jPosition)
    ...
    def elapseTime(self, gameState):
        ...
        for prior in self.particles:
            news = self.getPositionDistribution(gameState, prior)
            temp.append(news.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for val in set(self.particles):
            new[val] = belief_dist[val]
            for i in range(self.numGhosts):
                curr =val[i]
                new[val] *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), curr, self.getJailPosition(i)) 
        total = new.total()
        if total ==0:
            self.initializeUniformly(gameState) 
            return 
